...
def main():
    ...
    path1 = shortest_path_bfs(source, target)
    path2 = shortest_path_dfs(source, target)
    print("\nBFS Shortest Path\n")
    if path1 is None:
        print("Not connected.")
    else:
    ...
    if path2 is None:
        ...
        degrees = len(path2)
        print(f"{degrees} degrees of separation.")
        path2 = [(None, source)] + path2
        for i in range(degrees):
            person1 = people[path2[i][1]]["name"]
            person2 = people[path2[i + 1][1]]["name"]
            movie = movies[path2[i + 1][0]]["title"]
            print(f"{i + 1}: {person1} and {person2} starred in {movie}")
